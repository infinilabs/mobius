...
func execWatermarkAssetsTool(ctx context.Context, pg *PGClient, gcs *GCSClient, cfg *Config, events *EventPipeline, agentID string, args map[string]any) map[string]any {
	...
	if isBulk || isVideo {
		...
		if assigneeID != "" {
			t.Assignee = &EmployeeBrief{ID: assigneeID}
		}
		if agentID != "" {
			t.Creator = &EmployeeBrief{ID: agentID}
		}
		...
		if pg.esClient != nil {
			if full, err := pg.GetTask(ctx, t.ID); err == nil {
				if ierr := pg.esClient.IndexTask(ctx, full); ierr != nil {
					slog.Warn("ES index async watermark task failed", "id", t.ID, "error", ierr)
				}
			}
		}
		...
		if events != nil {
			...
			events.Publish(newEvent("watermark_job_created", aid, nil, nil, map[string]any{
...
func execVerifyWatermarkTool(ctx context.Context, gcs *GCSClient, cfg *Config, agentID string, args map[string]any) map[string]any {
	inputPath, _ := args["input_path"].(string)
	password, _ := args["password"].(string)
