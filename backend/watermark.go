...
func NewWatermarkEngine(sb SandboxConfig) *WatermarkEngine {
...
func (e *WatermarkEngine) EmbedImage(ctx context.Context, inputLocal, outputLocal, message, password string, intensity int) error {
	...
	stdout, stderr, exitCode, err := runSandboxedArgv(ctx, e.sb, workdir, argv, nil)
	if err != nil {
...
func (e *WatermarkEngine) ExtractVideoFrames(ctx context.Context, inputLocal, tmpDir string, fps float64) error {
	...
	workdir := filepath.Dir(inputLocal)
	_, stderr, exitCode, err := runSandboxedArgv(ctx, e.sb, workdir, argv, nil)
	if err != nil {
...
type watermarkDBClient interface {
	...
	AddTaskComment(ctx context.Context, taskID, authorID, content string) (*TaskComment, error)
...
func NewWatermarkTaskDriver(pg watermarkDBClient, gcs *GCSClient, engine *WatermarkEngine, actorID string, password string) *WatermarkTaskDriver {
