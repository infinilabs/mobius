// Package config is the application configuration (plan 6.4): the YAML
// schema, defaults, load/save, and the runtime-mutable settings view.
package config
...
// NsJailUsable mirrors the startup nsjail probe: true when the configured
// nsjail binary actually works on this host. Written by the sandbox probe,
// read by GetSettings to report the effective provider.
...
// AddModel appends a Vertex model registration under the config lock.
func (c *Config) AddModel(m VertexModel) {
	c.mu.Lock()
	defer c.mu.Unlock()
	c.GoogleCloud.VertexAI.Models = append(c.GoogleCloud.VertexAI.Models, m)
}

// RemoveModel deletes a Vertex model registration by ID under the config
// lock; reports whether it existed.
func (c *Config) RemoveModel(id string) bool {
	c.mu.Lock()
	defer c.mu.Unlock()
	models := c.GoogleCloud.VertexAI.Models
	for i, m := range models {
		if m.ID == id {
			c.GoogleCloud.VertexAI.Models = append(models[:i], models[i+1:]...)
			return true
		}
	}
	return false
