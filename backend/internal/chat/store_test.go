...
func TestConversationStore_AddMessage_AutoTitle(t *testing.T) {
	...
	s.AddMessage(c.ID, domain.Message{ID: "m1", Role: "user", Content: "How do I configure authentication?"})
...
func TestConversationStore_AddMessage_KeepsCustomTitle(t *testing.T) {
	...
	s.AddMessage(c.ID, domain.Message{ID: "m1", Role: "user", Content: "some question"})
...
func TestConversationStore_List_SortedByUpdatedAt(t *testing.T) {
	...
	s.AddMessage(c1.ID, domain.Message{ID: "m1", Role: "user", Content: "update c1"})
...
func TestConversationStore_TruncateAt(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		s.AddMessage(c.ID, domain.Message{ID: domain.NewID(), Role: "user", Content: "msg"})
	}
...
func TestConversationStore_TruncateAt_OverCount(t *testing.T) {
	s := NewConversationStore()
	c := s.Create()
	s.AddMessage(c.ID, domain.Message{ID: "m1", Role: "user", Content: "only one"})
