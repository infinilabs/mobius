...
func (f *fakeAuthzStore) GetEmployee(_ context.Context, id string) (*domain.Employee, error) {
...
func (f *fakeAuthzStore) GetTask(_ context.Context, id string) (*domain.Task, error) {
...
func (f *fakeAuthzStore) GetProject(_ context.Context, id string) (*domain.Project, error) {
...
func newHierarchyStore() *fakeAuthzStore {
	emp := func(id, role string, tags []string, managerID *string) *domain.Employee {
		return &domain.Employee{ID: id, Name: id, Role: role, Tags: tags, ManagerID: managerID}
	}
...
func TestCallerManages_CycleSafe(t *testing.T) {
	s := newHierarchyStore()
	s.employees["cyc-1"] = &domain.Employee{ID: "cyc-1", Role: "Custom", ManagerID: strPtr("cyc-2")}
	s.employees["cyc-2"] = &domain.Employee{ID: "cyc-2", Role: "Custom", ManagerID: strPtr("cyc-1")}
...
func TestRequireTaskAccess(t *testing.T) {
	s := newHierarchyStore()
	s.tasks["t1"] = &domain.Task{
		ID:       "t1",
		Creator:  &domain.EmployeeBrief{ID: "mgr-a"},
		Assignee: &domain.EmployeeBrief{ID: "worker-a1"},
	}
	ctx := context.Background()
...
func TestRequireProjectAccess(t *testing.T) {
	s := newHierarchyStore()
	s.projects["p1"] = &domain.Project{ID: "p1", Owner: &domain.EmployeeBrief{ID: "mgr-a"}}
	ctx := context.Background()
