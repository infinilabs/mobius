// Package events is the in-process event pipeline (plan 6.4b): buffered
// publish, batched flush to search/BQ sinks, and the nightly GCS archiver.
package events
...
// Sink receives flushed event batches (the ES mirror).
type Sink interface {
	BulkIndexEvents(ctx context.Context, events []*Event) error
}

// StreamSink receives flushed event batches as a stream insert (BigQuery).
type StreamSink interface {
	StreamInsertEvents(ctx context.Context, events []*Event) error
}

// ArchiveSource pages out stale events and prunes them after upload (ES).
type ArchiveSource interface {
	FetchEventsOlderThan(ctx context.Context, before time.Time, batchSize int) ([]Event, error)
	DeleteEventsByIDs(ctx context.Context, ids []string) (int, error)
}

// ArchiveStore persists gzipped archive objects (GCS). Pass nil-guarded
// implementations: a typed-nil defeats the pipeline's nil checks.
type ArchiveStore interface {
	Upload(ctx context.Context, prefix, fileID, ext string, data io.Reader, contentType string) (string, error)
}
...
func New(eventType string, actorID, projectID, taskID *string, payload map[string]any) *Event {
	return &Event{
		ID:        domain.NewID(),
		Timestamp: time.Now(),
		EventType: eventType,
...
func StartArchiver(ctx context.Context, cfg *config.Config, es ArchiveSource, gcs ArchiveStore) {
	if es == nil || gcs == nil {
...
func archiveEvents(ctx context.Context, es ArchiveSource, gcs ArchiveStore, retentionDays int, prefix string) {
...
// QueueStats reports the pipeline buffer's current length and capacity (for
// health reporting).
