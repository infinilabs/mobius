...
func (pg *Client) CreateInteraction(ctx context.Context, ix *domain.TaskInteraction) error {
	if ix.Payload == nil {
...
func (pg *Client) ListInteractions(ctx context.Context, taskID string) ([]domain.TaskInteraction, error) {
	...
	var interactions []domain.TaskInteraction
	for rows.Next() {
		var ix domain.TaskInteraction
	...
	if interactions == nil {
		interactions = []domain.TaskInteraction{}
	}
	return interactions, nil
}

func (pg *Client) ListResolvedInteractions(ctx context.Context, taskID string) ([]domain.TaskInteraction, error) {
	rows, err := pg.pool.Query(ctx, `
		SELECT id, task_id, creator_employee_id, kind, status, payload, response,
		       created_at, resolved_at, resolved_by
		FROM task_interactions
...
func (pg *Client) ResolveInteraction(ctx context.Context, interactionID string, resolvedBy string, response json.RawMessage) error {
	if response == nil {
...
func (pg *Client) DismissInteraction(ctx context.Context, interactionID string) error {
