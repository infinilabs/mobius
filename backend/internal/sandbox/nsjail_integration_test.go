...
func TestIntegration_NsJail(t *testing.T) {
	...
	sb := config.SandboxConfig{
		Enabled:    true,
		Provider:   config.ProviderNsJail,
		NsJailPath: "../bin/nsjail", // relative to backend/
	}
	Probe(sb)

	if !config.NsJailUsable.Load() {
...
func TestIntegration_DockerFallback(t *testing.T) {
	...
	oldUsable := config.NsJailUsable.Load()
	t.Cleanup(func() {
		config.NsJailUsable.Store(oldUsable)
	})
	...
	config.NsJailUsable.Store(false)
	Probe(sb)
	...
	stdout, stderr, code, err := RunCommand(ctx, sb, tmp, "echo 'fallback works'", nil)
	if err != nil {
