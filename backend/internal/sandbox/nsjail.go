...
func nsjailArgs(sb config.SandboxConfig, workdir, jailTmp, jailRoot string, env []string, timeLimitSec int) []string {
	args := []string{
...
func runNsJailCommand(ctx context.Context, sb config.SandboxConfig, workdir, command string, env []string) (stdout, stderr string, exitCode int, err error) {
...
func runNsJailArgv(ctx context.Context, sb config.SandboxConfig, workdir string, argv []string, env []string) (stdout, stderr string, exitCode int, err error) {
	nsjailPath := sb.NsJailPath
	if nsjailPath == "" {
		nsjailPath = "nsjail"
	}
...
// Probe runs once at boot. Proves nsjail can start a jail on this host.
// If not, sets config.NsJailUsable to false and falls back to Docker.
func Probe(sb config.SandboxConfig) {
	if sb.Provider != config.ProviderNsJail || runtime.GOOS != "linux" {
		return
	}
	...
	if err != nil || code != 0 || out != "ok\n" {
		slog.Warn("nsjail probe failed; falling back to Docker",
			"err", err, "code", code, "stderr", domain.TruncateStr(stderr, 500))
		return
	}
	config.NsJailUsable.Store(true)
