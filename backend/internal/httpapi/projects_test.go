...
func TestValidateProjectName_Template(t *testing.T) {
	...
	for _, name := range valid {
		if err := domain.ValidateProjectName(name, false); err != nil {
...
func TestValidateProjectName_Import(t *testing.T) {
	...
	for _, name := range valid {
		if err := domain.ValidateProjectName(name, true); err != nil {
...
func TestValidateProjectPath(t *testing.T) {
	...
	for _, p := range valid {
		if err := domain.ValidateProjectPath(p); err != nil {
...
func TestClassifyContentType(t *testing.T) {
	...
	for mime, expected := range cases {
		got := domain.ClassifyContentType(mime)
		if got != expected {
...
func TestResolveMimeType(t *testing.T) {
	...
	for _, tc := range cases {
		got := domain.ResolveMimeType(tc.filename, tc.header)
		if got != tc.expected {
...
func TestCalculateSHA256(t *testing.T) {
	hash := domain.CalculateSHA256([]byte("hello"))
	if len(hash) != 64 {
		t.Errorf("expected 64-char hex hash, got len=%d", len(hash))
	}

	empty := domain.CalculateSHA256([]byte{})
	if empty != "" {
		t.Errorf("expected empty string for empty input, got %q", empty)
	}

	h1 := domain.CalculateSHA256([]byte("same"))
	h2 := domain.CalculateSHA256([]byte("same"))
	if h1 != h2 {
		t.Error("identical inputs should produce identical hashes")
	}

	h3 := domain.CalculateSHA256([]byte("different"))
	if h1 == h3 {
...
func TestExportProjectToZip_DotfileExclusions(t *testing.T) {
	...
	project := &domain.Project{ID: "test-id", Name: "zip-test"}
	cfg := &config.Config{}
...
func TestProjectRootDir(t *testing.T) {
	...
	native := &domain.Project{Name: "my-app"}
	if got := native.RootDir(cfg.Projects.ProjectsDir); got != "/data/projects/my-app" {
		t.Errorf("native project rootDir = %q, want /data/projects/my-app", got)
	}

	src := "/home/user/repos/imported"
	imported := &domain.Project{Name: "imported", SourcePath: &src}
	if got := imported.RootDir(cfg.Projects.ProjectsDir); got != src {
