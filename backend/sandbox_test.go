...
func TestSandboxApplyDefaults_FillsZeroValues(t *testing.T) {
	...
	sb.applyDefaults()
	if sb.Image == "" || sb.Network == "" || sb.MemoryMB == 0 || sb.CPUs == "" || sb.PidsLimit == 0 {
		t.Errorf("applyDefaults left a zero value: %+v", sb)
	}
}

func TestSandboxApplyDefaults_PreservesExplicit(t *testing.T) {
	sb := SandboxConfig{Image: "custom:1", Network: "none", MemoryMB: 512, CPUs: "1", PidsLimit: 64}
	sb.applyDefaults()
	if sb.Image != "custom:1" || sb.Network != "none" || sb.MemoryMB != 512 || sb.CPUs != "1" || sb.PidsLimit != 64 {
