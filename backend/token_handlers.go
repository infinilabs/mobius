...
func (h *APIHandler) TokenSummary(w http.ResponseWriter, r *http.Request) {
	...
	ctx := r.Context()
	current, err := queryBQRow[summaryRow](ctx, h.bqClient, sql, params)
	if err != nil {
		writeError(w, "query failed: "+err.Error(), http.StatusInternalServerError)
		return
	}

	prev, _ := queryBQRow[prevRow](ctx, h.bqClient, prevSQL, prevParams)
...
func (h *APIHandler) TokenTimeseries(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := queryBQRows[tsRow](r.Context(), h.bqClient, sql, params)
	if err != nil {
		writeError(w, "query failed: "+err.Error(), http.StatusInternalServerError)
		return
	}
...
func (h *APIHandler) TokenBreakdown(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := queryBQRows[bdRow](r.Context(), h.bqClient, sql, params)
	if err != nil {
		writeError(w, "query failed: "+err.Error(), http.StatusInternalServerError)
		return
	}
...
func (h *APIHandler) TokenDetails(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := queryBQRows[detailRow](r.Context(), h.bqClient, sql, params)
	if err != nil {
		writeError(w, "query failed: "+err.Error(), http.StatusInternalServerError)
		return
	}
...
func queryBQRow[T any](ctx context.Context, bq *BQClient, sql string, params []bigquery.QueryParameter) (*T, error) {
...
func queryBQRows[T any](ctx context.Context, bq *BQClient, sql string, params []bigquery.QueryParameter) ([]T, error) {
	q := bq.client.Query(sql)
	q.Parameters = params
	it, err := q.Read(ctx)
	if err != nil {
